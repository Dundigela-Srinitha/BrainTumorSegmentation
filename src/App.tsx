...
//     formData.append('image', file); // Ensure the field name matches the backend
  
//     setIsProcessing(true);
//     setUploadedImage(URL.createObjectURL(file));
  
//     try {
//       const response = await axios.post('http://127.0.0.1:3000/api/segment', formData, {
...
function App() {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      // Create a URL for the segmented image blob
      const imageBlob = new Blob([response.data], { type: 'image/png' });
      const imageUrl = URL.createObjectURL(imageBlob);
  ...
  return (
    <div className="min-h-screen bg-[var(--medical-green-50)]">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        {/* Conditionally render Medical Stats only if no segmentation type is selected */}
        {!selectedType && (
          <div className="medical-stats">
            <div className="medical-stat-item">
              <Heart className="h-6 w-6 text-[var(--medical-green-500)] mb-2" />
